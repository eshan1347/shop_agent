...
async def get_product_page(pro_url:str, page: Any) -> Product | None:
    try:
        page.set_default_timeout(5000)        # 60s for all actions
        ...
        print(f"Error while fetching {pro_url} deets: {e}")
    ...
    try:
        ...
        for idx,tile in enumerate(tiles):
            try:
                ...
                print('ratings:', rat, num_rat, num_rev)
                #Highlights
                ...
                print('high:', high)
                # print(f"{idx}. {name} | {price} | {link} | {image} | {rat} | {num_rat} | {num_rev} | {high}")
                if name and price and link and image:
                    # print({'name': name, 'price': price, 'link': link, 'image': image, 'ratings': rat, 'num_ratings': num_rat, 'num_reviews': num_rev, 'mini_deets': high})
        ...
        print(f"Error during browser setup or navigation: {e}")
        ...
        try:
            ...
            for i in range(2, fcount):
                try:
                    ...
                    print(f'Processing filter {fname} ...')
                    is_exp = filter.locator('div.SDsN9S')
                    if await is_exp.count() < 1:
                        print(f'click click click ...')
                    ...
                    # print(f'Pre Model: name: {fname}, value: {filterval}')
            ...
            if user_filters:
                ...
                for i in range(2, fcount):
                    try:
                        ...
                        if vals:
                            for wanted in vals:
                                try:
                                    ...
                                    print(f"Selected {wanted} in {fname}")
                                except Exception as e:
                                    print(f"couldn't select {wanted} in {fname}: {e}")
                        ...
                        #         # await locator.click()
                        #         # print(f"Selected {filtername} in {fname} ...")
...
async def get_pro_links(base_url: str, page:Any) -> List[str]:
    ...
    for i,link_ele in enumerate(link_eles):
        try:
            ...
            print(f'Oopsie in get_pro_links: {e}')
