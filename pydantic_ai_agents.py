...
class Chatbot:
    ...
    def __init__(self, model_id: str = MODEL_ID, max_retries: int = MAX_RETRIES, top_k: int = 10):
        self.model_id = model_id
        self.max_retries = max_retries
        self.usage_limits = UsageLimits(request_limit=10, total_tokens_limit=20000)
        ...
        def sys_prompt0():
            ...
            Example workflow: 
            user query -> get_pro_class(user_query) -> prompt_user0(product_class) -> rephrase_query(query) -> get_best_site(product_class) -> get_site_filters(product_class, best_site) -> prompt_user1(site_filters, product_class) -> get_candidates(user_filters, search_query) -> final answer
            """
        ...
        async def get_pro_class(context: RunContext[ShopDeps]) -> None:
            ...
            """
            # print(f'node get_pro_class , deps: {context.deps}')
        ...
        async def rephrase_query(context: RunContext[ShopDeps]) -> None:
            ...
            """
            # print(f'node rephrase_query , deps: {context.deps}')
            context.deps.steps += 1
            query = context.deps.query
            res = await self.gen_agent.run(
                user_prompt=f"""
        ...
        async def prompt_user0(context: RunContext[ShopDeps]) -> None:
            ...
            # print(f'node prompt_user0 , deps: {context.deps}')
        ...
        async def get_best_site(context: RunContext[ShopDeps]) -> None:
            ...
            # print(f'node get_best_site , deps: {context.deps}')
            context.deps.steps += 1
            pro_class = context.deps.search_specs.pro_class
        ...
        async def get_site_filters(context: RunContext[ShopDeps]) -> None:
            ...
            """
            # print(f'node get_site_filters , deps: {context.deps}')
            context.deps.steps += 1
        ...
        async def prompt_user1(context: RunContext[ShopDeps]) -> None:
            ...
            """
            # print(f'node prompt_user1 , deps: {context.deps}')
            context.deps.steps += 1
            filters = context.deps.search_specs.site_filters
            if filters:
                res = ''
                ...
                print(f'Gen User filters: {user_filters}')
        ...
        async def get_candidates(context: RunContext[ShopDeps]) -> List[Product] | None:
            ...
            - End the user request & return them the relevant desired products
              """
            # print(f'node get_candidates , deps: {context.deps}')
            context.deps.steps += 1
    ...
    async def chat(self, user_query: str, searchspecs: SearchSpecs = SearchSpecs()) -> Any:
        ...
        print(f'Recommended product: {res.output.recommended} \n{res.output.message} \nAgent Workflow: {res.output.flow}')
